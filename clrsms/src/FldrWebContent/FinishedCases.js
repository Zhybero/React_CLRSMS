...
import { GetModeltblAppointmentsFinishedHearing, GetModelViewAPNameFinish, GetModeltblUserCredSpecific, GetImg, GetModeltblUserSpecific  } from "../FldrFunctions/ClsGetList";
...
import ClientInformationModalFinish from "../FldrModal/ClientInformationModalFinish";

function FinishedCases(){

    const [tblAppointmentsList, settblAppointmentsList] = useState([]);
    const [viewNameFinish, setviewNameFinish] = useState([]);
...
const fetchtblUserObj = async (varlist) => { 
  try {
    ...
    const fetchMdlAppointments = async (varUserCode) => {
      try {
        settblAppointmentsList(await GetModeltblAppointmentsFinishedHearing(varUserCode));
      } catch {
        console.log("Connection error!");
      }
    };
    const fetchMdlAppointmentsNameFinish = async () => {
      try {
        setviewNameFinish(await GetModelViewAPNameFinish()); 
      } catch {
        console.log("Connection error!");
      }
    };
    ...
    useEffect(() => {
      ...
      fetchMdlAppointmentsNameFinish();
    }, []);
    ...
    async function btnArchive(varlist){  
      const Mdl1 = { 
        ...
        try { 
          const response = await axios.put(`${PlsConnect()}/API/CLRSMSWEBAPI/UpdateModeltblAPArchive`, Mdl1, {
            headers: {
    ...
    let listAPList = searchTerm?searchList:viewNameFinish;
     
    const handleChange = (event) => {
      fetchMdlAppointmentsNameFinish(); 
      setSearchTerm(event.target.value); 
      const results = viewNameFinish.filter(
        (item) =>
    ...
    <div className="d-flex gap-1 align-items-center p-2 shadow">
      ...
      >
        <div className="modal-dialog modal-xl modal-dialog-centered modal-dialog-scrollable">
          <ClientInformationModalFinish
          vartblUser={tblUser}
          onSaveComplete={fetchMdlAppointmentsNameFinish} 
